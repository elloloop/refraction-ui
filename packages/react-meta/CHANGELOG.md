...
- 2154c6a: Fix: 0.23.0's ESM entry imported the unpublished `@refraction-ui/shared` (for the new `cn`/`cva` root exports), so importing `@refraction-ui/react` failed to resolve. The ESM split build now bundles every workspace package the entry re-exports and refuses to emit any `@refraction-ui/*` import.
...
- 4a20b5e: feat(react): `asChild` composition for Card, Badge, Callout, and LinkCard
...
- 59d1965: fix(astro): consumer builds no longer fail on optional analytics vendor SDKs
  ...
  React ripple: the React meta previously _vendored_ both vendor SDKs into its
...
- 5cfcf9d: fix(react,astro): route hardcoded status colors through theme tokens
...
- cdd92bc: Fix tree-shaking of the published meta package. Previously `import { Button } from '@refraction-ui/react'` bundled the entire library (494 kB minified) because tsup collapsed all ~128 adapters into a single `dist/index.js`, anchoring every component behind impure module-scope statements.
...
- 765ad2c: Fix six issues surfaced by the test-backfill waves:
...
- 0a1d8a9: Wire previously missing component packages into the framework metas so they actually reach consumers:
...
- caeeb87: SlideViewer: arrow-key navigation (ArrowRight/ArrowLeft) now actually re-renders the component — previously the headless state advanced and `onSlideChange` fired, but the slide counter, progress bar, content, and button states stayed stale until the next click.
...
- 3ccc1b3: SlideViewer: arrow-key navigation (ArrowRight/ArrowLeft) now actually re-renders the component — previously the headless state advanced and `onSlideChange` fired, but the slide counter, progress bar, content, and button states stayed stale until the next click.
...
- f5a21d7: Composer: full emoji experience, WhatsApp-style expression panel, and motion polish.
...
- c443a6c: Add Composer — a headless-core chat composer (`RefractionComposer`) with auto-grow and viewport cap, IME-safe Enter-to-send with Shift+Enter newline, `@`/`/`/`:`/`#`/custom trigger suggestion menus with atomic mention/emoji/tag tokens and structured output, grapheme-safe max-length clamping, typed attachments with paste/drag-drop capture, draft persistence, edit-in-place, busy/stop state for AI surfaces, density-aware theming, RTL, and full combobox/listbox ARIA. Also generalizes trigger detection into a single `detectTriggerInText` in `rich-editor` (consumed by mentions, slash commands, and the conversation composer).
...
- b76f773: Composer polish: replace the text/emoji toolbar labels with proper Lucide-style SVG icons (paperclip · B / I / code / link · quote / bulleted / numbered list), regroup with a vertical divider, swap the send button to a circular arrow-up (`disabled` state shown subtly). Cleaner default placeholder ("Send a message…"); the `/`, `@`, `:` trigger menus are unchanged behaviourally.
...
- 8dc1ca8: RSC: every published `@refraction-ui/react` JS entry (`.`, `./theme`,
...
- Add voice-focused primitives for AI and audio interfaces.

  - `VoicePill` adds a fixed-position speaker status pill with safe-area-aware placement, intensity-driven pulse rings, speaker-keyed theming, polite live-region metadata, and an optional icon mute toggle.
  - `Waveform` adds a canvas waveform visualizer for analyser nodes, media streams, sample arrays, and low-cost intensity streams with resize-aware rendering, smoothing, paused state, reduced-motion behavior, and bars / line / rings variants.
