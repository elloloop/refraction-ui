...
- 48ef17e: - **DropdownMenu**: `DropdownMenuTrigger` now implements `asChild` — it merges the trigger's props and toggle behavior onto the child element instead of rendering a `<button>` around it (nested buttons are invalid HTML and caused a hydration error on the docs page). The `asChild` prop no longer leaks to the DOM.
