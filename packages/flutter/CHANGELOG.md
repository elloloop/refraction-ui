...
- **Avatar: real initials, a deterministic tint, presence.** `RefractionAvatar` now shows the first letter of the first and last words (`"Jane Doe"` → `"JD"`, was `"JA"`; single words give one letter; short upper-case initials like `"JD"` are kept as given; grapheme-aware) — `RefractionAvatar.initialsFor`, matching React's `getInitials`. The fallback sits on a tint picked from the name (stable FNV-1a hash into an 8-hue ramp rotated from the `info` token, so monochrome brands still get color), with initials pushed to AA contrast; override with `tint`, or pass `colorSeed` (e.g. a user id) so a rename keeps the color. New `presence` puts a ringed `RefractionPresenceIndicator` on the bottom-end corner (`ringColor` = the surface). Initials show while a network image loads, not an empty disc. The avatar is one semantics image labelled with the name (and presence), overridable via `semanticLabel`. New `resized()`.
- **AvatarGroup:** members keep their own image, tint and presence (the group used to rebuild them, dropping everything but image/text/radius); the overlap accounts for the ring; new `ringColor`; the stack is one semantics element announced as `"Ana, Ben and 3 others"` (`RefractionAvatarGroup.describe`, or `semanticLabel`); the `+N` chip uses `foreground` (the old `mutedForeground` on `muted` missed AA).
- **PresenceIndicator:** colors come from theme tokens (`online` → `positive`, `away` → `caution`, `busy`/`dnd` → `destructive`, `offline` → `neutral`) via `RefractionPresenceIndicator.colorFor`; status is no longer color-only — `away`/`offline` are hollow rings and `dnd` has a bar. New `diameter` and `ringColor`. `defaultColors` is deprecated.
