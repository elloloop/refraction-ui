...
- **Feature — work-tracking task list**: `RefractionTaskList`, `RefractionTaskGroup`, `RefractionTaskRow`, `RefractionCompletionCheck` and the pure `RefractionDueDates` helpers — a monday-style grouped to-do list.
  - `RefractionTaskGroup`: collapsible group ("Overdue", "Today", "Upcoming", "Done") with a coloured title, a count chip, optional trailing action and empty label; header is a button with `expanded` semantics; collapsed rows leave the tree, so Tab and screen readers never reach hidden content; controlled or uncontrolled.
  - `RefractionTaskRow`: completion check, title, source link, owner slot, due/age text with urgency tone, status slot (typically `RefractionStatusPill`). Responsive: aligned trailing columns ≥ 640 px, two-line compact layout below. Completion plays a disc pop + check draw + strike + success tint at once and reports `onCompletedChanged` after `completionSettle` (default 650 ms) so hosts can move it to "Done" after the user sees it land; immediate under reduced motion; still reported if the row is removed mid-settle.
  - Keyboard (desktop/web): each row is one Tab stop; Space completes, Enter opens; inside a `RefractionTaskList` ↑/↓/Home/End move between rows and group headers across groups and scroll them into view.
  - `RefractionCompletionCheck`: round animated check with hover preview, ≥ 32 px hit target, focus ring and checkbox semantics.
  - `RefractionDueDates.group/bucketOf/describe`: deterministic ("now" is an argument) Overdue/Today/Upcoming/No date/Done bucketing and "Today"/"Tomorrow"/"3d overdue"/"in 4d"/"Oct 9" labels with a tone.
- **Feature — `RefractionStatusPill`**: a colour-coded work-status chip (the "status column" of a work-tracking board). `solid` (saturated fill) and `soft` (tinted fill + dot) variants, `sm`/`md` sizes, `expand` for table cells, optional icon, custom `color` for host-defined statuses. Colours come from the status tokens via `type`; the label ink is resolved per surface so it always meets WCAG AA (4.5:1) on every curated palette — saturated hues are deepened to carry white ink, light hues (yellows) keep their fill and take dark ink. `onPressed` makes it a focusable button (Enter/Space, hover, keyboard focus ring, `button` semantics, chevron hint); status changes cross-fade and snap under reduced motion. `RefractionStatusPillColors.resolve` exposes the resolved fill/ink for matching swatches.
- **Fix — `RefractionStatusIndicator` colours are now tokens**: the six hardcoded Tailwind hexes are replaced by the status tokens (`positive`, `destructive`, `caution`, `info`, `pending`, `neutral`) through a new shared `RefractionStatusType.colorIn(colors)` mapping, so one token swap re-skins every status surface. The label uses an AA-legible shade of `mutedForeground` (the minimal palettes' `#8E8E93` is 3.3:1 on white). Visual note: with the default tokens `pending` now follows `warning` (amber) instead of orange-500, and `neutral` follows `mutedForeground`.
