...
- **Toasts rebuilt: `RefractionToaster` + `RefractionToastController` (backward compatible).** `RefractionToast.show` used to insert a lone `OverlayEntry` fixed at bottom-right, so multiple toasts overlapped, a 320-px toast clipped on phones, nothing could be undone or dismissed, and screen readers never heard it. Now:
  - `variant` (`neutral`/`success`/`info`/`warning`/`destructive`, with icon + AA status tint), an `action` (`RefractionToastAction` — "Undo", "Retry"), a close button, `onDismissed`, and `show()` returns a `RefractionToastHandle` (`dismiss()`, `isActive`) — e.g. dismiss the "Retrying…" toast when the send succeeds.
  - Toasts stack (newest nearest the bottom edge, `maxVisible` 3, oldest retired first); phone widths (< 600) span the bottom edge above the safe area, wider screens stack bottom-right at 360.
  - Timing: 4 s default, 6 s with an action, `Duration.zero` = until dismissed; the timer **pauses while hovered or focused** (WCAG 2.2.1). Swipe to dismiss; Escape dismisses a focused toast; each toast is a live region; entrance honours reduced motion.
  - Put a `RefractionToaster` in `MaterialApp.builder` (or pass your own controller to show toasts from non-UI code). Without one, `show()` installs a toaster in the root overlay, so existing call sites keep working — and now stack.
- **Fix (a11y): curated palettes meet WCAG AA for every base text pair.** Adjusted only the values that failed: `mutedForeground` (minimal/fintech/wellness/creative/productivity light, wellness dark — was 2.9–4.4:1), `destructive` fills that could not carry white text (minimal, fintech, creative light, wellness — now red-600-class hues, was 3.2–4.2:1), `primaryForeground` on neon/coral brands (fintechLight, wellnessLight now use dark ink — was 1.97 / 2.74:1), `productivityLight.primary` (blue-600), `creativeDark.primary` (a hair darker, was 4.47:1) and `secondaryForeground` in wellness/creative dark. A palette-wide test now asserts AA for foreground/card/popover/primary/secondary/destructive/accent/muted pairs on all 22 palettes.
- **Fix (a11y): `positiveForeground`, `cautionForeground`, `pendingForeground` default to the AA ink** (`successForeground`/`warningForeground`) instead of white — white on the default amber was 2.15:1, on green 2.28:1. Explicit overrides are unaffected.
- **Fix (a11y): `RefractionButton`, `RefractionCheckbox`, `RefractionRadio`, `RefractionSwitch` are keyboard-operable.** They were `GestureDetector`s: unreachable with Tab, no Enter/Space, no focus indicator. They now take focus in order, activate with Enter/Space, draw the theme focus ring in keyboard mode only (never on click), expose a semantics `onTap`, skip focus when disabled, and honour reduced motion. New optional `semanticLabel` on all four (required in practice for icon-only buttons — otherwise announced as just "button"), plus `focusNode`/`autofocus` on `RefractionButton`.
- **Fix: `RefractionButton` hugs its content.** A `Container.alignment` made every button stretch to the full available width inside a `Column`/`Wrap`/`Center` (a row of buttons rendered as full-width bars). Buttons in a tight/stretch context still fill it.
- **Fix: icon children of `RefractionButton` take the variant's foreground color** (they kept the ambient icon color, which is near-invisible on dark ghost/outline buttons).
- **New: `RefractionPressable`** — the shared focus/keyboard/hover/press layer those controls use (Tab, Enter/Space, focus ring, hover/pressed state to a builder, no semantics of its own). Use it for custom interactive rows and icon buttons.
- **Fix:** `RefractionInput`, `RefractionTextarea`, `RefractionOtpInput`, `RefractionDatePicker` and `RefractionSkipToContent` draw their focus state in `focusRing` (3:1 guaranteed) instead of the raw `ring` (1.5:1 in `minimalLight`).
