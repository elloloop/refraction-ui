...
void main() {
  ...
  group("RefractionSteps State and Color Evaluation", () {
    testWidgets('evaluates correct visual color state for step 0 when currentStep is 0', (tester) async {
      final steps = List.generate(3, (i) => RefractionStepItem(title: Text('Step ${i+1}')));
      await tester.pumpWidget(buildApp(RefractionSteps(steps: steps, currentStep: 0)));
    ...
    testWidgets('evaluates correct visual color state for step 1 when currentStep is 0', (tester) async {
      final steps = List.generate(3, (i) => RefractionStepItem(title: Text('Step ${i+1}')));
      await tester.pumpWidget(buildApp(RefractionSteps(steps: steps, currentStep: 0)));
      // Since colors are read via context theme inside the builder, we verify the presence without crash.
      // And we verify default indicator numbers exist.
    ...
    testWidgets('evaluates correct visual color state for step 2 when currentStep is 0', (tester) async {
      final steps = List.generate(3, (i) => RefractionStepItem(title: Text('Step ${i+1}')));
      await tester.pumpWidget(buildApp(RefractionSteps(steps: steps, currentStep: 0)));
      // Since colors are read via context theme inside the builder, we verify the presence without crash.
      // And we verify default indicator numbers exist.
    ...
    testWidgets('evaluates correct visual color state for step 0 when currentStep is 1', (tester) async {
      final steps = List.generate(3, (i) => RefractionStepItem(title: Text('Step ${i+1}')));
      await tester.pumpWidget(buildApp(RefractionSteps(steps: steps, currentStep: 1)));
      // Since colors are read via context theme inside the builder, we verify the presence without crash.
      // And we verify default indicator numbers exist.
      expect(find.text('${0 + 1}'), findsOneWidget);
    });
      
    testWidgets('evaluates correct visual color state for step 1 when currentStep is 1', (tester) async {
      final steps = List.generate(3, (i) => RefractionStepItem(title: Text('Step ${i+1}')));
      await tester.pumpWidget(buildApp(RefractionSteps(steps: steps, currentStep: 1)));
      // Since colors are read via context theme inside the builder, we verify the presence without crash.
      // And we verify default indicator numbers exist.
    ...
    testWidgets('evaluates correct visual color state for step 2 when currentStep is 1', (tester) async {
      final steps = List.generate(3, (i) => RefractionStepItem(title: Text('Step ${i+1}')));
      await tester.pumpWidget(buildApp(RefractionSteps(steps: steps, currentStep: 1)));
      // Since colors are read via context theme inside the builder, we verify the presence without crash.
      // And we verify default indicator numbers exist.
    ...
    testWidgets('evaluates correct visual color state for step 0 when currentStep is 2', (tester) async {
      final steps = List.generate(3, (i) => RefractionStepItem(title: Text('Step ${i+1}')));
      await tester.pumpWidget(buildApp(RefractionSteps(steps: steps, currentStep: 2)));
      // Since colors are read via context theme inside the builder, we verify the presence without crash.
      // And we verify default indicator numbers exist.
      expect(find.text('${0 + 1}'), findsOneWidget);
    });
      
    testWidgets('evaluates correct visual color state for step 1 when currentStep is 2', (tester) async {
      final steps = List.generate(3, (i) => RefractionStepItem(title: Text('Step ${i+1}')));
      await tester.pumpWidget(buildApp(RefractionSteps(steps: steps, currentStep: 2)));
      // Since colors are read via context theme inside the builder, we verify the presence without crash.
      // And we verify default indicator numbers exist.
    ...
    testWidgets('evaluates correct visual color state for step 2 when currentStep is 2', (tester) async {
      final steps = List.generate(3, (i) => RefractionStepItem(title: Text('Step ${i+1}')));
      await tester.pumpWidget(buildApp(RefractionSteps(steps: steps, currentStep: 2)));
      // Since colors are read via context theme inside the builder, we verify the presence without crash.
      // And we verify default indicator numbers exist.
    ...
    testWidgets('evaluates correct visual color state for step 0 when currentStep is 3', (tester) async {
      final steps = List.generate(3, (i) => RefractionStepItem(title: Text('Step ${i+1}')));
      await tester.pumpWidget(buildApp(RefractionSteps(steps: steps, currentStep: 3)));
      // Since colors are read via context theme inside the builder, we verify the presence without crash.
      // And we verify default indicator numbers exist.
      expect(find.text('${0 + 1}'), findsOneWidget);
    });
      
    testWidgets('evaluates correct visual color state for step 1 when currentStep is 3', (tester) async {
      final steps = List.generate(3, (i) => RefractionStepItem(title: Text('Step ${i+1}')));
      await tester.pumpWidget(buildApp(RefractionSteps(steps: steps, currentStep: 3)));
      // Since colors are read via context theme inside the builder, we verify the presence without crash.
      // And we verify default indicator numbers exist.
    ...
    testWidgets('evaluates correct visual color state for step 2 when currentStep is 3', (tester) async {
      final steps = List.generate(3, (i) => RefractionStepItem(title: Text('Step ${i+1}')));
      await tester.pumpWidget(buildApp(RefractionSteps(steps: steps, currentStep: 3)));
      // Since colors are read via context theme inside the builder, we verify the presence without crash.
      // And we verify default indicator numbers exist.
