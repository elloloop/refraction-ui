...
void main() {
  ...
  group('RefractionWaveform', () {
    testWidgets('1. renders silent waveform without samples', (WidgetTester tester) async {
      await tester.pumpWidget(buildApp(
        const RefractionWaveform(paused: true),
      ));
      expect(find.byType(RefractionWaveform), findsOneWidget);
    });

    for (var variant in WaveformVariant.values) {
      testWidgets('renders variant ${variant.name}', (WidgetTester tester) async {
        await tester.pumpWidget(buildApp(
          RefractionWaveform(variant: variant, paused: true),
        ));
        expect(find.byType(RefractionWaveform), findsOneWidget);
      });
    }
    ...
    for (var length in testLengths) {
      testWidgets('handles sample length $length', (WidgetTester tester) async {
        ...
        await tester.pumpWidget(buildApp(
          RefractionWaveform(samples: samples, paused: true),
        ));
        expect(find.byType(RefractionWaveform), findsOneWidget);
      });
    }
    ...
    for (var count in barCounts) {
      testWidgets('handles barCount $count', (WidgetTester tester) async {
        await tester.pumpWidget(buildApp(
          RefractionWaveform(barCount: count, paused: true),
        ));
        expect(find.byType(RefractionWaveform), findsOneWidget);
      });
    }
    ...
    for (var intensity in intensities) {
      testWidgets('handles intensity $intensity', (WidgetTester tester) async {
        await tester.pumpWidget(buildApp(
          RefractionWaveform(intensity: intensity, paused: true),
        ));
        expect(find.byType(RefractionWaveform), findsOneWidget);
      });
    }
    ...
    for (var amplitude in amplitudes) {
      testWidgets('handles amplitude $amplitude', (WidgetTester tester) async {
        await tester.pumpWidget(buildApp(
          RefractionWaveform(amplitude: amplitude, paused: true),
        ));
        expect(find.byType(RefractionWaveform), findsOneWidget);
      });
    }
    ...
    for (var smoothing in smoothings) {
      testWidgets('handles smoothing $smoothing', (WidgetTester tester) async {
        await tester.pumpWidget(buildApp(
          RefractionWaveform(smoothing: smoothing, paused: true),
        ));
        expect(find.byType(RefractionWaveform), findsOneWidget);
      });
    }
    ...
    testWidgets('handles paused state changes', (WidgetTester tester) async {
      ...
      await tester.pumpWidget(buildApp(
        const RefractionWaveform(paused: false),
      ));
    ...
    testWidgets('respects explicit width and height', (WidgetTester tester) async {
      await tester.pumpWidget(buildApp(
    ...
    testWidgets('falls back to default height and full width', (WidgetTester tester) async {
      await tester.pumpWidget(buildApp(
        const RefractionWaveform(paused: true),
      ));
      final sizedBox = tester.widget<SizedBox>(find.ancestor(
    ...
    testWidgets('respects custom color', (WidgetTester tester) async {
      await tester.pumpWidget(buildApp(
        const RefractionWaveform(color: Colors.red, paused: true),
      ));
      expect(find.byType(RefractionWaveform), findsOneWidget);
    });

    testWidgets('handles samples with NaN and Infinity', (WidgetTester tester) async {
      await tester.pumpWidget(buildApp(
        const RefractionWaveform(
          samples: [double.nan, double.infinity, double.negativeInfinity, 0.0],
          paused: true,
        ),
      ));
      expect(find.byType(RefractionWaveform), findsOneWidget);
    ...
    testWidgets('handles dynamic sample updates', (WidgetTester tester) async {
      await tester.pumpWidget(buildApp(
        const RefractionWaveform(samples: [0.1, 0.2], paused: false),
      ));
      await tester.pump(const Duration(milliseconds: 16));
      await tester.pumpWidget(buildApp(
