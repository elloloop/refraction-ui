...
export function ThemePlayground() {
  ...
  const generateDart = useCallback(() => {
    const colorLines = Object.entries(colorVars)
      .map(([key, value]) => {
        const dartKey = key.replace('--', '').replace(/-([a-z])/g, (g) => g[1].toUpperCase())
        return `    ${dartKey}: HslColor.parse('${value}'),`
      })
      .join('\n')

    const radiusDouble = parseFloat(currentTheme.radius) * 16 || 8

    return `final myTheme = RefractionThemeData(
  borderRadius: ${radiusDouble},
...
${colorLines}
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Generated Dart */}
      <div className="space-y-4">
        <h2 className="text-lg font-semibold text-foreground">Generated Dart Code</h2>
        <div className="rounded-lg border border-border bg-card overflow-hidden">
          <div className="flex items-center justify-between border-b border-border bg-muted/30 px-4 py-2">
            <span className="text-xs font-mono text-muted-foreground">dart</span>
